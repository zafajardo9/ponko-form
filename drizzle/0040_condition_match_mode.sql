ALTER TABLE "form_page_fields"
ADD COLUMN IF NOT EXISTS "condition_match" varchar(10) DEFAULT 'all' NOT NULL;
--> statement-breakpoint
ALTER TABLE "form_page_fields"
DROP CONSTRAINT IF EXISTS "form_page_fields_condition_match_check";
--> statement-breakpoint
ALTER TABLE "form_page_fields"
ADD CONSTRAINT "form_page_fields_condition_match_check"
CHECK ("condition_match" IN ('all', 'any'));
--> statement-breakpoint
-- Keep the match mode when the builder atomically replaces the page structure.
...
BEGIN
  ...
  LOOP
    INSERT INTO "form_pages" (
      ...
      "final_template", "final_redirect_url", "final_contact_email", "has_payment",
      ...
      NULLIF(v_page->>'finalContactEmail', ''),
    ...
    LOOP
      INSERT INTO "form_page_fields" (
        ...
        CASE WHEN v_field->>'conditionMatch' = 'any' THEN 'any' ELSE 'all' END
