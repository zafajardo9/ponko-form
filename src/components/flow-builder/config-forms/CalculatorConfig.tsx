...
export function CalculatorConfig({ nodeId, config, variables, onChange }: ConfigFormProps) {
    ...
    setTestValues((prev) => {
      ...
      for (const name of referenced) {
        if (next[name] === undefined) {
          next[name] = variables.find((v) => v.name === name)?.defaultValue ?? ''
        }
      }
      return next
    })
  ...
  const targetType = variables.find((v) => v.name === config.targetVariable)?.type
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field label="Expression" hint="Combine variables, numbers, operators (+ − × ÷) and functions.">
        <textarea
          ref={taRef}
          value={expr}
          rows={3}
      ...
      <div className="rounded-lg border border-[#e6dfd8] bg-white">
        ...
        >
          <span className="text-sm font-medium text-[#141413]">Test calculation</span>
          <span className="text-xs text-[#8e8b82]">{showTester ? '▲ Hide' : '▼ Show'}</span>
        </button>
                    ...
                    <label key={name} className="flex items-center gap-2">
                      ...
                      {v?.type === 'boolean' ? (
                        <select
                          ...
                          onChange={(e) =>
                            setTestValues((s) => ({ ...s, [name]: e.target.value }))
                          }
                        ...
                        <input
                          ...
                          type={v?.type === 'number' || v?.type === 'money' ? 'number' : 'text'}
                ...
                {testResult.success ? (
                  <span className="flex items-baseline gap-1.5">
                    ...
                    <span className="font-semibold">
                      {config.targetVariable ? `${config.targetVariable} = ` : ''}
                      {isMoneyTarget && '₱'}
 ...
 */
function ExpressionPreview({ expr, variables }: { expr: string; variables: FlowVariable[] }) {
  ...
  function pushText(text: string) {
    if (!text) return
    const pretty = text.replace(/\*/g, ' × ').replace(/\//g, ' ÷ ').replace(/\s+/g, ' ')
    parts.push(
  ...
  return <div className="flex flex-wrap items-center gap-y-1 font-mono text-sm leading-6">{parts}</div>
}
