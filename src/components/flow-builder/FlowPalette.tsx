...
export const PALETTE_ITEMS: {
  ...
  { type: 'form_field', label: 'Form Field', icon: '☐', accent: 'bg-[#dbe7f7] text-[#2f5a9e]', description: 'Collect input from the user' },
...
export function FlowPalette({ onAddNode }: FlowPaletteProps) {
  return (
    <aside className="flex flex-col gap-2">
      <p className="text-xs font-medium uppercase tracking-wider text-[#8e8b82]">Node Types</p>
      ...
      <p className="mt-2 px-1 text-xs text-[#8e8b82]">Drag onto the canvas, or click to add.</p>
    </aside>
  )
}
