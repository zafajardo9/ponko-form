  ...
  return (
    <aside className="flex flex-col gap-4 lg:gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="-mx-1 grid max-h-44 grid-cols-2 gap-2 overflow-y-auto px-1 sm:grid-cols-3 lg:mx-0 lg:max-h-none lg:grid-cols-1 lg:overflow-visible lg:px-0">
          {FIELD_ITEMS.map(({ fieldType, label, icon }) => (
            <button
              ...
              className="flex min-w-0 items-center gap-2 rounded-lg border border-[#e6dfd8] bg-[#faf9f5] px-3 py-2.5 text-left text-sm transition-colors hover:border-[#cc785c] hover:bg-[#efe9de] active:bg-[#e8e0d2] lg:gap-3"
            >
              <span className="flex h-7 w-7 flex-none items-center justify-center rounded-md bg-[#efe9de] text-[#cc785c]">
                {icon}
              </span>
              <span className="min-w-0 truncate text-[#141413]">{label}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 lg:mx-0 lg:flex-col lg:overflow-visible lg:px-0 lg:pb-0">
          {LOGIC_ITEMS.map(({ type, label, icon, accent, description }) => (
            <button
              ...
              className="flex min-w-[168px] items-start gap-3 rounded-lg border border-[#e6dfd8] bg-[#faf9f5] px-3 py-2.5 text-left transition-colors hover:border-[#cc785c] hover:bg-[#efe9de] active:bg-[#e8e0d2] lg:min-w-0"
            >
              <span
              ...
              <span className="min-w-0">
                <span className="block truncate text-sm text-[#141413]">{label}</span>
      ...
      <p className="hidden px-1 text-xs text-[#8e8b82] lg:block">
