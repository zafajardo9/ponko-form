...
import { X, AlertTriangle, Play, SkipBack, RotateCcw } from "lucide-react";
  ...
  return (
    <PanelFrame onClose={onClose}>
      <div className="flex items-center justify-between">
        <span className="rounded bg-[#f7ecd0] px-1.5 py-0.5 text-[11px] font-medium text-[#9e7424] flex items-center gap-1">
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          size="sm"
          onClick={back}
          disabled={engine.getCurrentStepNumber() <= 1}
        >
          Back
        </Button>
      ...
      <div className="mt-2 border-t border-[#e6dfd8] pt-3">
        ...
        <div className="mt-2 flex flex-col gap-1">
          {variables.length === 0 && (
            <span className="text-xs text-[#8e8b82]">None declared.</span>
          )}
          {variables.map((v) => (
            <div
              key={v.id}
  ...
  return (
    <aside className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onClose}
          className="text-sm text-[#8e8b82] hover:text-[#141413]"
        >
          <X size={16} />
        </button>
      </div>
...
function StepBody({
    ...
    return (
      <div className="rounded-lg border border-[#e6dfd8] bg-white p-3">
        <p className="text-sm font-medium text-[#141413]">
          {(config.title as string) ?? "Summary"}
        </p>
  ...
  if (complete && step.nodeType === "redirect") {
    return (
      <div className="rounded-lg border border-[#e6dfd8] bg-white p-3 text-sm text-[#57544d]">
        Would redirect to:{" "}
        <span className="break-all font-mono text-[#141413]">
          {step.redirectUrl}
        </span>
      </div>
    );
  ...
  if (step.nodeType === "form_field") {
    ...
    const options =
      (config.options as { label: string; value: string }[] | undefined) ?? [];
    return (
        ...
        {fieldType === "textarea" ? (
          ...
          <div className="flex flex-col gap-1.5">
            {options.map((o) => (
              <label
                key={o.value}
                className="flex items-center gap-2 text-sm text-[#141413]"
              >
                <input
                  type="radio"
                  name="preview-field"
  ...
  if (step.nodeType === "group") {
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        {fields.length === 0 && (
          <p className="text-xs text-[#8e8b82]">No fields in this group yet.</p>
        )}
        {fields.map((f) => {
            ...
            <div key={f.id} className="flex flex-col gap-1.5">
              ...
              {f.fieldType === "textarea" ? (
                <textarea
                  value={val}
                  onChange={(e) =>
                    setGroupInput({ ...groupInput, [f.id]: e.target.value })
                  }
                  className="rounded-md border border-[#e6dfd8] bg-[#faf9f5] px-3 py-2 text-sm"
                  rows={2}
                />
                  ...
                  {options.map((o) => (
                    ...
                    >
                      <input
                        ...
                        checked={val === o.value}
                        onChange={() =>
                          setGroupInput({ ...groupInput, [f.id]: o.value })
                        }
                        className="accent-[#cc785c]"
                      />
                      {o.label}
      ...
      <div className="flex flex-col gap-2">
        ...
        {branches.map((b) => (
          ...
          >
            ...
            />
            {b.label}{" "}
            <span className="text-xs text-[#8e8b82]">({b.value})</span>
          </label>
        ))}
      </div>
    );
  ...
  if (step.isPayment) {
    ...
    return (
      <div className="flex flex-col gap-3">
        <div className="rounded-lg border border-[#e6dfd8] bg-white p-3 text-sm text-[#57544d]">
          [TEST] Would charge{" "}
          <span className="font-medium text-[#141413]">{amountVar}</span> (
          {currency}).
        </div>
