...
export function IntegrationModal({ provider, open, onClose, onSave, onOAuth, saving, error, configured, meta }: IntegrationModalProps) {
  ...
  useEffect(() => {
    ...
    void getGoogleAuthUrl()
      .then((res) => {
        if (!cancelled && res.url) setOauthUrl(res.url)
      })
      .catch((oauthLoadError: unknown) => {
        if (cancelled) return
        console.error('[ponkoform-integrations] Failed to load Google OAuth URL', oauthLoadError)
        setOauthError('Google connection is temporarily unavailable. Save your settings and try again.')
      })
    return () => {
      cancelled = true
    }
        ...
        <div className="flex flex-col gap-4 overflow-y-auto p-5">
          ...
          {isOAuth ? (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3.5">
                {cfg.fields.map((field) => (
                  <div key={field.name} className="flex flex-col gap-1">
                    ...
                    />
                    {field.docLink && (
                      <a href={field.docLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-[#a9583e] hover:text-[#7f402e]">
                        Where to find this
              ...
              ) : configured && oauthError ? (
                <div className="rounded-lg border border-[#e3b5ab] bg-[#fbf3f0] px-4 py-3 text-sm text-[#9f3f35]" role="alert">
                  {oauthError}
                </div>
              ) : configured && !oauthUrl ? (
                <div className="rounded-lg border border-[#e6dfd8] bg-[#faf9f5] px-4 py-3 text-sm text-[#6c6a64]">
              ...
              <div key={field.name} className="flex flex-col gap-1">
                ...
                {field.docLink && (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-xs font-medium text-[#a9583e] hover:text-[#7f402e]"
                  >
                    Where to find this
                    <ExternalLink size={11} aria-hidden="true" />
                  </a>
