...
function GoogleCallbackPage() {
  ...
  const search = useSearch({ from: Route.id }) as { code?: string; provider?: string }
  ...
  useEffect(() => {
    ...
    import('../../../lib/server-fns/google-oauth').then(async ({ handleGoogleCallback }) => {
      try {
        const result = await handleGoogleCallback({ data: { code: search.code! } })
        if (result.success) {
          setStatus('success')
      ...
      <Card className="text-center py-12">
        ...
        {status === 'success' && (
          <>
            ...
            <p className="mt-2 text-sm text-[#6c6a64]">{message}</p>
            <div className="mt-6">
              <Button onClick={() => navigate({ to: '/dashboard/integrations' })}>
        ...
        {status === 'error' && (
          <>
            <div className="mb-4 text-5xl">❌</div>
            <h1 className="text-xl font-medium text-[#141413]">Connection failed</h1>
            <p className="mt-2 text-sm text-[#c64545]">{message}</p>
            <div className="mt-6 flex items-center justify-center gap-3">
              <Button variant="secondary" onClick={() => navigate({ to: '/dashboard/integrations' })}>
                Back
              </Button>
