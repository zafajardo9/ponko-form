...
import { formSubmissions, forms, formFields, flows, flowNodes, profiles } from '../../db/schema'
  ...
  if (flow) {
    ...
    for (const node of nodes) {
      ...
      if (node.type === 'form_field') {
        ...
        if (key) columns.push({ key, label: (config.label as string) || node.label || key })
      } else if (node.type === 'group') {
        const fields = (config.fields as { label?: string; bindToVariable?: string }[] | undefined) ?? []
        for (const f of fields) {
          if (f.bindToVariable) columns.push({ key: f.bindToVariable, label: f.label || f.bindToVariable })
        }
      }
    }
    return columns
...
export const submitFormResponse = createServerFn({ method: 'POST', strict: false })
  .inputValidator((data: { formId: number; formData: Record<string, unknown> }) => data)
  .handler(async ({ data }) => {
...
export const getSubmissions = createServerFn({ method: 'GET', strict: false })
  ...
  .handler(async ({ data }) => {
    ...
    return { submissions, columns, form }
  })
